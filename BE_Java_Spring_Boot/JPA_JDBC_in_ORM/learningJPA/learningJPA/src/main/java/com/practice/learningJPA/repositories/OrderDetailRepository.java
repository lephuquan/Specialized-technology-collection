...
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Long> {
    ...
    //@NamedQuery
    OrderDetail findByBookNameAndOrderId(@Param("bookName") String bookName,@Param("authorId") Long authorId);// Query duoc goi tu @NamedQuery cua  OrderDetail entity, nên dùng @Param để chỉ định tham số - ko có vẫn được

    @Query(value = "SELECT o.id as id, o.book as book, o.order as order FROM OrderDetail o where o.book.title = :bookName and o.order.id = :authorId")// Tìm nạp phép chiếu Proxy interface
    IGetBookDetail findByBookNameAndOrderIdCase2(@Param("bookName") String bookName,@Param("authorId") Long authorId);
